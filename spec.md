...
3. **生成中画面**
   - ローディングアニメーションと「コーディネートを生成中…」メッセージ
4. **結果画面**
   - 生成された全身写真を中央に表示
   - 「もう一度作る」ボタン
...
- 背景はシンプルな単色またはスタジオ風
- モデル外見は MVP では固定:
  - `a slim East Asian person, neutral expression, full body shot`
...
```text
A full-body fashion photo of a slim East Asian person, neutral expression, wearing a {color_hat} {hat}, {color_eyewear} {eyewear}, {color_outer} {outer}, {color_tops} {tops}, {color_pants} {pants}, and {color_shoes} {shoes}. Clean background, high detail, realistic lighting, full body shot.
```
...
- キー: 正規化したプロンプト文字列の SHA-256 ハッシュ
- 値: 生成画像の URL と有効期限
- 有効期限: 24 時間（無料枠を圧迫しない程度）
