...
| 画像生成モデル | FLUX（Pollinations `gen.pollinations.ai/image/{prompt}`）            |
...
- **採用 API**: Pollinations の無料エンドポイントを使用
  - エンドポイント: `https://gen.pollinations.ai/image/{prompt}`
  - 認証: Pollinations API Key（`POLLINATIONS_API_KEY`、開発者が無料で発行。オプション）
- **Rate Limit 対策**:
  - 生成リクエストを単純な順次処理とし、超過時は「しばらくお待ちください」表示を出す
  - MVP ではキャッシュ・キューは導入しない
- **フォールバック**: Pollinations の FLUX 画像生成はキーなしでも無料で利用可能
...
**重要**: GitHub Pages だけでは API キーを隠せないため、Pollinations API はブラウザから直接呼び出さず、Cloudflare Pages Functions 経由で呼び出す。`POLLINATIONS_API_KEY` は Workers の環境変数（シークレット）として管理する。
...
[Cloudflare Pages Functions] ── API プロキシ
       │
       ├── Pollinations API Key（シークレット、オプション）
       │
       ▼
[Pollinations AI]
       │
...
1. リクエストをバリデーション
2. プロンプトを構築
3. Pollinations 画像生成 API を呼び出し（縦長 768×1344 で生成）
...
MVP では無料かつ個人利用を想定し **FLUX.1-schnell** を使用する。Pollinations 経由で `FLUX` モデルが利用可能。
...
| Day 2 | 数時間  | Pages Functions 経由で Pollinations API 連携、プロンプト生成実装 |
...
| リスク                 | 内容                  | 対策                          |
| ------------------- | ------------------- | --------------------------- |
| Pollinations 無料枠の制限 | Rate Limit や待ち時間が発生 | `POLLINATIONS_API_KEY` を使用、シンプルなエラーメッセージ |
| 画像生成の品質ブレ           | 自由入力によりプロンプトが不安定    | プロンプト例を UI に表示、全身・縦長を強調   |
| API キーの漏洩         | ブラウザに露出すると危険        | Cloudflare Pages Functions で秘匿管理    |
